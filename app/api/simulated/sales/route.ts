...
import { salesNotes } from "@/lib/seed/salesNotes";
...
export async function POST() {
    ...
    const result = await prisma.feedback.createMany({
        data: salesNotes.map((n) => ({
            content: n.content,
            channel: "Email",
            customerLabel: n.customerLabel,
    ...
    // Fire-and-forget AI classification for every imported record
    classifyBatch(session.user.workspaceId, result.count, insertedAt).catch((err) =>
        console.error("[AI] Sales batch classify error:", err)
    );
